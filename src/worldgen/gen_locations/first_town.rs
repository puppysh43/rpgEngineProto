...
pub fn first_town() -> Location {
    ...
    let north = Map::from_string(north_raw);
    ...
    let center = Map::from_string(center_raw);
    ...
    let east = Map::from_string(east_raw);
    ...
    let south = Map::from_string(south_raw);
    let spawn_pos = Point::new(25, 0);
    let mut first_town = Location::new(north, spawn_pos);
